                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="ticket_code_mode" id="ticket_code_mode" class="form-control">
                                    @php
                                        ...
                                        $ticketCodeMode = in_array($ticketCodeModeRaw, ['shared', 'unique'], true) ? $ticketCodeModeRaw : 'unique';
                                    @endphp
                                    <option value="unique" {{ $ticketCodeMode === 'unique' ? 'selected' : '' }}>Kode Berbeda Semua (Unik)</option>
                                    <option value="shared" {{ $ticketCodeMode === 'shared' ? 'selected' : '' }}>Kode Sama per Jenis Ticket</option>
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="ticket_print_orientation" id="ticket_print_orientation" class="form-control">
                                    @php
                                        ...
                                        if ($ticketPrintOrientationRaw === 'portrait') $ticketPrintOrientationRaw = 'with_summary';
                                        elseif ($ticketPrintOrientationRaw === 'portrait_with_first_qr') $ticketPrintOrientationRaw = 'without_summary';
                                        $ticketPrintOrientation = in_array($ticketPrintOrientationRaw, ['with_summary', 'without_summary'], true) ? $ticketPrintOrientationRaw : 'without_summary';
                                    @endphp
                                    <option value="without_summary" {{ $ticketPrintOrientation === 'without_summary' ? 'selected' : '' }}>Tidak Dicetak (Default)</option>
                                    <option value="with_summary" {{ $ticketPrintOrientation === 'with_summary' ? 'selected' : '' }}>Dicetak</option>
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="dashboard_metric_mode" id="dashboard_metric_mode" class="form-control">
                                    @php $dashboardMetricMode = $setting->dashboard_metric_mode ?? old('dashboard_metric_mode', 'amount'); @endphp
                                    <option value="amount" {{ $dashboardMetricMode === 'amount' ? 'selected' : '' }}>Tampilkan Jumlah Uang</option>
                                    <option value="count" {{ $dashboardMetricMode === 'count' ? 'selected' : '' }}>Tampilkan Jumlah Transaksi</option>
                                </select>
                            ...
                            <div class="form-group mb-3">
                                <label for="ticket_scan_cooldown_seconds">Jeda Scan Tiket</label>
                                <div class="input-group">
                                    <input type="number" name="ticket_scan_cooldown_seconds" id="ticket_scan_cooldown_seconds" class="form-control" value="{{ $setting->ticket_scan_cooldown_seconds ?? old('ticket_scan_cooldown_seconds', 0) }}" min="0">
                                    <span class="input-group-text">Detik</span>
                                </div>
                                <small class="text-muted">Jeda minimal antar scan dengan kode tiket yang sama. Contoh: 120 = harus menunggu 2 menit. Isi 0 jika tanpa jeda.</small>
                                @error('ticket_scan_cooldown_seconds') <br><small class="text-danger">{{ $message }}</small> @enderror
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-lg-10">
                            <div class="form-group mb-3">
                                ...
                                <input type="text" name="renewal_notice_club_name" id="renewal_notice_club_name" class="form-control" value="{{ old('renewal_notice_club_name', $setting->renewal_notice_club_name ?? 'MIU') }}">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="text" name="renewal_notice_admin_phone" id="renewal_notice_admin_phone" class="form-control" value="{{ old('renewal_notice_admin_phone', $setting->renewal_notice_admin_phone ?? '0821 2222 9358') }}">
...
<script>
    $(document).ready(function() {
        function togglePreviewVisibility() {
            ...
            if (isChecked && hasImage) $('#preview-container').fadeIn();
            else $('#preview-container').fadeOut();
        }
